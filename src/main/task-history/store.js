...
let persistTimer;
let activeWrite;
function getTaskHistoryPath() {
    return path.join(app.getPath('userData'), TASK_HISTORY_FILE);
}
function taskSortValue(task) {
    return Math.max(task.finishedAt ?? 0, task.startedAt ?? 0, task.logs[task.logs.length - 1]?.at ?? 0);
}
    ...
    return {
        ...
        result: task.result && typeof task.result === 'object'
            ? {
                ...
                prNumber: typeof task.result.prNumber === 'number' && Number.isFinite(task.result.prNumber)
...
function markRestoredTask(task, now) {
...
export function scheduleTaskHistoryPersist(tasks) {
    latestTasks = tasks;
    pendingTasks = tasks;
    if (persistTimer) {
...
export async function loadTaskHistory() {
    ...
    try {
        ...
        return restoreTasks(parsed.tasks.map((task) => normalizeTask(task)).filter((task) => Boolean(task)));
    }
    catch (error) {
...
export function flushTaskHistorySync() {
    if (persistTimer) {
        clearTimeout(persistTimer);
        persistTimer = undefined;
    }
