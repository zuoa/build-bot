...
function buildAuthedCloneUrl(context) {
...
function summarizeCloneFailure(output) {
...
export async function listChangedFiles(workspacePath) {
...
function truncateDiffText(diff) {
...
function buildTextFileCreationDiff(file, content) {
...
async function buildUntrackedFileDiff(workspacePath, file) {
...
async function getTrackedFileDiff(workspacePath, file) {
    const git = simpleGit(workspacePath);
    return git.raw([
        'diff',
        '--no-color',
...
export async function buildTaskFileChanges(workspacePath, files) {
...
export async function getFileDiffSummary(workspacePath, files) {
    if (files.length === 0) {
        return '无文件变更';
    }
    const git = simpleGit(workspacePath);
