...
let cachedCapabilities: ClaudeCapabilities | undefined;
...
function extractJsonCandidate(line: string): string | undefined {
...
function parseEventObject(line: string): Record<string, unknown> | undefined {
...
function parseStreamEvent(line: string): StreamParseResult {
  ...
  if (type === 'system' && subtype === 'init') {
    ...
    const failedMcp = mcpServers.filter(
      (item) => isRecord(item) && typeof item.status === 'string' && item.status === 'failed'
    ).length;
    return {
      recognized: true,
      log: {
...
function buildSpawnPlan(claudeArgs: string[], usePty: boolean): SpawnPlan {
...
async function runClaudeTaskOnce(
  params: {
    cwd: string;
    prompt: string;
    taskType: TaskType;
...
export async function runClaudeTask(params: {
  ...
  const runWithPtyFallback = async (leanStartup: boolean): Promise<void> => {
    try {
